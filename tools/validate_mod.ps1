<#
.SYNOPSIS
    FS25_FuelCosts Mod Validator (PowerShell version)
...
Write-Header "FS25_FuelCosts Mod Validator"
...
foreach ($file in $LuaFiles) {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        ...
        if (-not $isComment -and $line -match '\bprint\s*\(' -and $line -notmatch 'FuelCosts\.log' -and $RelPath -notmatch 'FuelCostsCore\.lua') {
            Add-Warning -File $RelPath -Category "DEBUG_CODE" -Line $lineNum -Message "Raw print() - use FuelCosts.log* instead"
        }
    }
}
...
# Only flag fc_* keys as missing (base game keys are expected to be in game's i18n)
foreach ($file in $LuaFiles) {
    $RelPath = $file.FullName.Substring($ModPath.Length + 1)
    $content = Get-Content $file.FullName -Raw -ErrorAction SilentlyContinue
    if (-not $content) { continue }
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        ...
        foreach ($match in $keyMatches) {
            ...
            # Only flag fc_* keys as errors (base game keys are assumed to exist)
            # Skip keys ending with _ (dynamic key construction like "fc_fluid_" .. fluidType)
            if ($key.StartsWith('fc_') -and -not $key.EndsWith('_') -and $key -notin $TransKeys) {
