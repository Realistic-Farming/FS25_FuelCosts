<#
.SYNOPSIS
    Extracts FuelCosts-related errors and warnings from FS25 game log.

.DESCRIPTION
    Parses the Farming Simulator 25 log.txt file and extracts all entries
    related to the FuelCosts mod, categorizing them by severity and type.
...
Write-Header "FuelCosts Log Analyzer"
...
# Patterns to match
$patterns = @{
    # FuelCosts specific patterns
    FuelCostsLine = 'FuelCosts|FS25_FuelCosts'
    ModLoaded = 'v(\d+\.\d+\.\d+)'
...
foreach ($i in 0..($lines.Count - 1)) {
    ...
    # Check if line is FuelCosts related
    if ($line -match $patterns.FuelCostsLine) {
        $stats.FuelCostsLines++
    ...
    if ($line -match $patterns.XmlParse -and $line -match 'FuelCosts') {
        $stats.XmlErrors += @{
            Line = $i + 1
            Text = $line.Trim()
            Type = "XmlError"
...
$report += "FuelCosts-Related Lines: $($stats.FuelCostsLines)"
